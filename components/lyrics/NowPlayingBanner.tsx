...
export default function NowPlayingBanner({ playing, seekVersion, onControl, controlLoading, scopeError, onSeek }: Props) {
  ...
  return (
    <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900">
      {/* Top row: album art + track info + controls (desktop only) */}
      <div className="flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          {/* Seek bar — desktop only (sits under artist name) */}
        ...
        {/* Controls — desktop only (right side) */}
        <div className="hidden sm:flex items-center gap-1 shrink-0">
          {controlButtons}
        </div>
      </div>

      {/* Controls — mobile only (centered row, below track info) */}
      <div className="sm:hidden flex items-center justify-center gap-1 mt-2">
        {controlButtons}
      </div>

      {/* Seek bar — mobile only (below controls) */}
